...
export default function ProfilePage() {
  ...
  const getVerified = async () => {
    try {
      ...
      setSending(true);
      const res = await axios.post("/api/users/getexistuserverified", {
        email: data.email,
      });
  ...
  return (
    <div className="flex  border justify-center min-h-screen bg-cover p-16 gap-8"
    style={{ backgroundImage: `url("/bg.png")` }}>
      
        {/* PROFILE - user information */}
        <div className="flex flex-col items-center justify-center ">
          <h1 className="text-center w-full text-white font-semibold text-2xl">
            My Profile
          </h1>
          <hr />
            ...
            {data.email === "" ? (
              ...
              <div>
                ...
                <p className="bg-[#ffffff52] w-full py-1 px-2 ">
                  Verified:{" "}
                  {data.isVerified === false || sending === false ? (
                    <span>
                      You are not verified; <br />
                      <a
                        className="font-italic underline text-blue-300 "
                        onClick={getVerified}
                      >
                        Get verified now!
                      </a>
